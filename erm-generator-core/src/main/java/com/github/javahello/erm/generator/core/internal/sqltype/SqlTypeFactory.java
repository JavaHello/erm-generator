...
import com.github.javahello.erm.generator.core.util.POIUtils;
...
public class SqlTypeFactory {
	...
	public static void load() throws IOException, ClassNotFoundException {
		...
		try {
			...
			Map<String, Map<SqlType.TypeKey, SqlType>> dbSqlTypeMap = new HashMap<String, Map<SqlType.TypeKey, SqlType>>();
			...
			for (int colNum = 4; colNum < headerRow.getLastCellNum(); colNum += 6) {
				...
				dbSqlTypeMap.put(dbId, new LinkedHashMap<SqlType.TypeKey, SqlType>());
			}
			...
			for (int rowNum = 1; rowNum <= sheet.getLastRowNum(); rowNum++) {
				...
				if (DiffHelper.isEmpty(sqlTypeId)) {
					break;
				}
				...
				for (int colNum = 4; colNum < row.getLastCellNum(); colNum += 6) {
					...
					if (POIUtils.getCellColor(sheet, rowNum, colNum) != HSSFColor.GREY_50_PERCENT.index) {
						...
						if (!DiffHelper.isEmpty(alias)) {
							...
							if (!DiffHelper.isEmpty(aliasForConvert)) {
